#+begin_src latex
    ...
    length, which can be set by the user, $g_6$ might now also intersect
    ...
    which is $g_3$ in our example. From a given starting gene, $g_i$, we
    move to the right to find all overlapping genes. Since any gene
    $g_{i+n}$ can overlap the focus interval, it might seem we have to
    scan all genes to the right of $g_i$. However, we can do better by
    realizing that we can stop once we have found a gene whose start is
    further from $e$ than the length of the longest gene in the
    list. This means we need for each chromosome the length of the
    longest gene it contains.
...
#+end_src
#+begin_export latex
We store genes as slices of pointers to intervals indexed by their
chromosome. So we construct such a map between chromosome names and
intervals before reading the genes. In addition, we store the maximum
gene lengths in a map indexed by chromosome names. So we construct
such a map before calculating the maximum gene lengths.
#+end_export
...
#+begin_src latex
  ...
  determines the sorting order. As explained at the beginning of this
...
parsed by the function \ty{scan}, which takes the variable holding the
genes and their maximum lengths as arguments. The intersecting
intervals are written to a table using a buffered writer. So we
construct a buffered writer, which we also pass as an argument of
\ty{scan}. The table has a header, which we construct and pass. The
format of the table we print is determined by \ty{-c}, which we pass
as well.
#+end_export
...
with the current chromosome. Then we search for the leftmost gene
whose end is greater than the start of the focus interval, $g_3$ in
Figure~\ref{fig:iv}.
#+end_export
...
encounter along the way are superset of the desired IDs. So before
starting our walk along the slice of genes, we empty the list of IDs
obtained in the last round and then collect those we encounter during
our walk.
#+end_export
