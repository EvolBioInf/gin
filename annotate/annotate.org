#+begin_src latex
    ...
    of gene IDs, \ty{annotate} can also return the gene names, or
    ``symbols''. And instead of output in four ann, \ty{annotate} can
    return IDs or symbols in a single column.
    ...
    interval $[s,e]$, and the six genes, $g_1,...,g_6$ in
    Figure~\ref{fig:iv}. The focus interval intersects four genes, $g_5$,
    $g_4$, $g_3$, and $g_2$. However, by default \ty{annotate} intersects
    promoters rather than whole genes. Promoters are modeled as intervals
    of some constant length upstream of the transcription start
    site. Depending on the promoter length, which can be set by the user,
    $g_6$ might now also intersect the interval, and so might $g_1$, as it
...
#+begin_src latex
  ...
  first of these is interpreted as the GFF file of annotations,
  especially gene annotations. If the GFF file doesn't exists, we read
  it; if not, we bail with a friendly message. Otherwise, we read
  it. But before we embark on this potentially time-consuming task, we
  check that all files supplied by the user exist. Then the genes are
  read into slices of intervals indexed by the sequences they belong
  to. When we're done with the GFF file, we reduce the file list to the
  interval files (if any).
...
#+begin_src go <<Parse input files, Ch.~\ref{ch:an}>>=
  ...
  //<<Check existence of additional files, Ch.~\ref{ch:an}>>
...
#+end_src
#+begin_export latex
We check the existence of all files supplied by the user.
#+end_export
#+begin_src go <<Check existence of additional files, Ch.~\ref{ch:an}>>=
...
#+end_src
#+begin_src latex
  Genes are modeled as intervals. So before we read the genes, we
  declare an interval as consisting of a start, an end, and an
  identifier.
#+end_src
...
#+end_src
#+begin_src latex
  As described in the Introduction, column 3 in a GFF file states the
  annotation type. We're only interested in genes. From the gene
  annotations we determine the gene interval before adding it to the
  appropriate slice.
#+end_src
...
#+begin_src latex
  ...
  for picking intersecting genes. Consider again the focus interval
  $[s,e]$ and the genes $g_1,...,g_6$ in Figure~\ref{fig:iv}. Moving
  from left to right, we search for the first gene that intersects the
  interval, $g_2$. Then we walk to the right until we find the first
  gene that starts to the right of $e$, $g_6$. This requires that we
  sort the genes with two keys. The primary key is the start position,
  by which we sort them in ascending order. The secondary key is the end
  position, by which we also sort them in ascending order, as shown for
  $g_1$ and $g_2$, which start at the same position, but $g_1$ ends
  earlier.
#+end_src
...
#+end_src
#+begin_src latex
  We retrieve the four arguments we passed, the map between chromosomes
  and genes, the output writer, the header for each set of annotations,
  and whether or not we print the output in a single column.
#+end_src
