#+begin_src latex
  ...
  browser, for example \ty{hsRefGene.txt} available from
  \begin{verbatim}
  ...
  single interval, and Table~\ref{tab:an}C shows the result when
  intersecting the interval with the genes. Multiple sets of intervals
  are separated by single blanks.
...
#+begin_src latex
  ...
  column, \ty{-c}. There is also fast gene lookup, which relies on genes
  not being proper subintervals of each other. This is not strictly
...
#+begin_src go <<Declare options, Ch.~\ref{ch:an}>>=
  ...
  var optS = flag.Bool("s", false, "gene symbols instead of " +
	  "accessions")
  var optC = flag.Bool("c", false, "print accessions or symbols " +
	  "in a single column")
  var optF = flag.Bool("f", false, "fast gene lookup")
#+end_src
#+begin_src latex
  We import \ty{flag}.
#+end_src
...
#+begin_src latex
  ...
  construct one and also pass it as the an argument of \ty{scan}. The
  table has a header, which we construct and pass as argument. The
  format of the table we print is determined by \ty{-c}, so we pass that
  as well. During the scan can may use the fast method for gene lookup
  (\ty{-f}), our last option.
#+end_src
#+begin_src go <<Parse input files, Ch.~\ref{ch:an}>>=
  ...
  clio.ParseFiles(files, scan, genes, w, header, *optC, *optF)
#+end_src
#+begin_src latex
  We import \ty{log} and \ty{bufio}.
#+end_src
#+begin_src go <<Imports, Ch.~\ref{ch:an}>>=
  "log"
  "bufio"
...
#+end_src
#+begin_src latex
  A row in the output table either ends in symbols or in accessions. We
...
#+end_src
#+begin_src latex
  We either use the faster method for looking up the intersecting genes
  or the slower, more accurate one. Both start from the first position
  beyond the last intersecting gene, from where we move to the
  left. Accessions are separated by commas, except for the first one,
  which is separated by a tab. So we count the intersecting genes to
  distinguish between the first and all subsequent ones.
#+end_src
#+begin_src go <<Collect accessions, Ch.~\ref{ch:an}>>=
  ...
  i--
  gc := 0
  //<<Fast search, Ch.~\ref{ch:an}>>
  if ! fast {
	  //<<Slow search, Ch.~\ref{ch:an}>>
  }
#+end_src
#+begin_src latex
  In the fast search, we walk to the left until we reach the first gene
  whose end is to the left of the intersection start.
#+end_src
#+begin_src go <<Fast search, Ch.~\ref{ch:an}>>=
  for i >= 0 && g[i].end >= s {
	  gc++
	  if !gm[g[i].acc] {
...
#+end_src
#+begin_src latex
  We end the fast search on a non-overlapping gene. But there is no
  guarantee that there aren't more overlapping genes somewhere to the
  left of that. So the slow search continues the search to the first
  gene in the list. This is bound to be excessive, but guaranteed to be
  correct.
#+end_src
#+begin_src go <<Slow search, Ch.~\ref{ch:an}>>=
  i--
  for i >= 0 {
	  if g[i].start <= e && g[i].end >= s {
		  gc++
		  if !gm[g[i].acc] {
			  //<<Add accession, Ch.~\ref{ch:an}>>
		  }
...
#+end_src
#+begin_src latex
  We run four tests. The first three are on the large data set in
  \ty{i1.txt}, the fourth on two segments out of this data set to test
...
#+begin_src go <<Construct tests, Ch.~\ref{ch:an}>>=
  ...
  tests = append(tests, test)
  test = exec.Command("./annotate", "-s", "-f", g, i1)
  tests = append(tests, test)
