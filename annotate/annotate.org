#+begin_src latex
    ...
    interval $[s,e]$, and the seven genes, $g_1,...,g_7$ in
	...
	\input{../annotate/iv}
      \end{center}
      \caption{Cartoon of a focus interval, $[s,e]$, and seven genes,
	$g_1,...,g_7$; the program \ty{annotate} finds the genes, that
...
#+begin_src latex
  ...
  interval, i.e. the first gene whose end is greater than $s$, which is
  $g_2$. Then we walk to the right until we find the first gene that
  starts to the right of $e$, $g_6$. This requires that we sort the
  genes with two keys. The primary key is the end position, by which we
  sort them in ascending order. The secondary key is the start position,
  by which we also sort them in ascending order.
#+end_src
