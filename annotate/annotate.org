...
#+begin_src latex
  ...
  especially gene annotations. If the GFF file exists, we read it; if
  not, we bail with a friendly message. The genes are read into slices
  of intervals indexed by the sequences they belong to.
...
#+begin_src go <<Store gene, Ch.~\ref{ch:an}>>=
  ...
  if fields[2] == "gene" {
	  ...
	  //<<Add gene to interval slice, Chr.~\ref{ch:an}>>
  }
#+end_src
#+begin_src latex
  We import \ty{strings}.
...
#+end_src
#+begin_src latex
  We extract the gene information consisting of transcription start and
  end, strand, and ID.
#+end_src
...
#+end_export
#+begin_src go <<Add gene to interval to slice, Chr.~\ref{ch:an}>>=
