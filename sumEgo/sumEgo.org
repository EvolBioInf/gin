...
data doesn't, but consists of at least seven fields. If we find a
...
#+end_src
#+begin_export latex
A header line consists of seven entries, for example,
\begin{verbatim}
#GO O E O/E P(n=1.0e+02) Category Description
\end{verbatim}
The fifth entry, \verb-P(n=1.0e+02)-, indicates the number of
iterations, which we extract and add to the total number of
iterations.
#+end_export
